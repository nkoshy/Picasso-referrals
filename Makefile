...
VERSION_PKG = github.com/PicassoExchange/picasso-referals/version
...
solidity-wrappers: $(SOLIDITY_DIR)/contracts/*.sol
	...
	for file in $(^F) ; do \
			...
			echo abigen --type=token --pkg wrappers --out=../wrappers/$${file}/wrapper.go --sol $${file} ; \
			abigen --type=token --pkg wrappers --out=../wrappers/$${file}/wrapper.go --sol $${file} ; \
	done
